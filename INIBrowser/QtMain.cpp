...
int main(int argc, char* argv[])
{
    ...
    // 官方建议（Qt 6.4+ Scene Graph 文档）：QSG_NO_VSYNC 关闭 vsync 后，默认动画驱动
    // 依赖渲染循环持续刷帧才推进，不再产生中间帧 → NumberAnimation 直接瞬跳。
    // 设置 QSG_USE_SIMPLE_ANIMATION_DRIVER=1 改用基于定时器的简单动画驱动，
    // 使标准 QML 动画（NumberAnimation 等）在无 vsync 下也能正常产生渐变帧。
