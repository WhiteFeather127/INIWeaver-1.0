...
public:
    ...
    // 对应 ImGui 立即模式下每帧重算 DefaultCenter。返回写回所用的 sessionId（QML 侧
    // 诊断 [COMP-LIVE] 用 projectSessionCenter 做"实际位置 vs 世界缓存投影"实时对账）。
    Q_INVOKABLE qulonglong setLinkNodeCenter(int row, qreal x, qreal y);
    // IIF 分量节点坐标回写：compIdx 定位分量，sessionId 按 compIdx 重算（对应 UpdateAll 用 Comp=cidx）
    // 写回后连线端点表经 priority 2（sv.LastCenter）解析到正确分量圆点
    Q_INVOKABLE qulonglong setLinkNodeCenterAt(int row, int compIdx, qreal x, qreal y);
    // IIF 分量节点坐标回写（按 keyName 稳定定位）：row 索引在 rebuildEntries 重建后可能错位，
    // 用 keyName 查 lineIdx（稳定）避免 sessionId 算错导致连线端点漂移
    Q_INVOKABLE qulonglong setLinkNodeCenterAtKey(const QString &keyName, int lineMult, int compIdx, qreal x, qreal y);
